...
void main() {
      ...
      vec4 rectColor = mix(gradientColor, u_colorBorder, borderAlpha);

      vec4 bgWithShadow = mix(u_colorBg, u_colorShadow, shadowAlpha);


      vec4 finalColor = mix(bgWithShadow, rectColor, smoothedAlpha);
